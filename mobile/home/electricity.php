...
<script>
    document.addEventListener("DOMContentLoaded", function(){
        const meterNumberInput = document.querySelector('input[name="meternumber"]');
        const meterNameDisplay = document.createElement('div');
        meterNameDisplay.id = 'meter-name-display';
        meterNumberInput.parentNode.appendChild(meterNameDisplay);

        meterNumberInput.addEventListener('input', function(){
            const meternumber = this.value;

            meterNameDisplay.textContent = '';

            if(meterNumber) {
                const formData = new FormData();
                formData.append('verify-meter-no', true);
                formData.append('meternumber', meterNumber);

                fetch('confirm-electricity', {
                    method: 'POST',
                    body:formData
                })
                ...
                .then(data => {
                    if(data[1].status === 'success') {
                        meterNameDisplay.textContent = `Meter Name: ${data[1].msg}`;
                    } else {
                        meterNameDisplay.textContent = 'Meter not found.';
                    }
                })
                .catch(error => {
